...
<?php 
    ...
    $qbarang = mysqli_query($con,"select penjualan.id_penjualan, penjualan.time, barang.kd_barang, barang.nm_barang, barang.id_barang, penjualan.jumlah_jual, barang.satuan, penjualan.harga_jual, pembelian.harga_beli, penjualan.total_harga, penjualan.diskon from penjualan join barang on penjualan.id_barang = barang.id_barang join pembelian on penjualan.id_barang = pembelian.id_barang where barang.id_jenis = '$param_jenis' group by penjualan.id_barang");
    while($br_jual = mysqli_fetch_array($qbarang)){
        ...
        $q_jumlah_jual = mysqli_query($con,"select sum(jumlah_jual) from penjualan where id_barang = '$id_barang' ");
        $jumlah_jual=mysqli_fetch_array($q_jumlah_jual);
        
        //diskon per item
        $q_diskon=mysqli_query($con,"select sum(total_diskon) from penjualan where id_barang = '$id_barang' ");
...
$st_qharga_jual = mysqli_query($con,"select sum(total_harga_jual) from penjualan join barang on penjualan.id_barang = barang.id_barang where barang.id_jenis = '$param_jenis' ");
$st_harga_jual = mysqli_fetch_array($st_qharga_jual);

// subtotal harga pokok per jenis
$st_qharga_beli = mysqli_query($con,"select sum(total_harga_beli) from penjualan join barang on penjualan.id_barang = barang.id_barang where barang.id_jenis = '$param_jenis' ");
$st_harga_beli = mysqli_fetch_array($st_qharga_beli);

// subtotal diskon per jenis
$st_qdiskon = mysqli_query($con,"select sum(total_diskon) from penjualan join barang on penjualan.id_barang = barang.id_barang where barang.id_jenis = '$param_jenis' ");
...
$st_qterjual = mysqli_query($con,"select sum(jumlah_jual) from penjualan join barang on penjualan.id_barang = barang.id_barang where barang.id_jenis = '$param_jenis' ");
...
<br>
    <?php } 
        ...
        $q_terjual= mysqli_query($con,"select sum(jumlah_jual) from penjualan ");
        $total_terjual = mysqli_fetch_array($q_terjual);

        //total harga jual
        $q_harga_jual = mysqli_query($con,"select sum(total_harga) from penjualan ");
        $total_harga = mysqli_fetch_array($q_harga_jual);

        //total harga beli
        $q_harga_beli = mysqli_query($con,"select sum(total_harga_beli) from penjualan ");
        $total_harga_beli = mysqli_fetch_array($q_harga_beli);

        //total diskon
        $q_total_diskon = mysqli_query($con,"select sum(total_diskon) from penjualan ");
